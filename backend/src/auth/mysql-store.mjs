...
`;
  ...
  async function addDisputeEvidence(input) {
    ...
    const fileUrl = normalizeOptionalString(attachment?.url) ?? (attachment?.fileId ? `/api/files/${attachment.fileId}` : null);
    const sql = `
...
SELECT JSON_OBJECT(
  ...
  'disputeCount', (SELECT COUNT(*) FROM \`dispute\` WHERE \`status\` NOT IN ('resolved', 'cancelled')),
...
SET @provider_wallet_updated = ROW_COUNT();
SET @updated = IF(@payer_wallet_updated = 1 AND @provider_wallet_updated = 1, 1, 0);
UPDATE \`dispute\` SET \`status\` = 'resolved', \`final_result\` = @final_result, \`refund_amount\` = @refund_amount, \`resolution_note\` = @reason, \`updated_at\` = @resolved_at, \`resolved_at\` = @resolved_at WHERE \`dispute_id\` = @dispute_id AND @updated = 1 LIMIT 1;
  ...
  'userGrowth', (
    ...
    FROM (
      SELECT
        ...
        COUNT(*) AS \`new_users\`,
        (SELECT COUNT(*) FROM \`user\` ux WHERE DATE_FORMAT(ux.\`created_at\`, '%Y-%m') <= DATE_FORMAT(ANY_VALUE(u.\`created_at\`), '%Y-%m')) AS \`total_users\`
      FROM \`user\` u
...
FROM (
  SELECT
    l.*,
    DATE_FORMAT(l.\`created_at\`, '%Y-%m-%dT%H:%i:%s.000') AS \`call_created_at\`,

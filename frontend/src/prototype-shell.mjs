...
function bindAdminLoginForm() {
  ...
  // 绑定角色选择器点击事件
  const roleOptions = document.querySelectorAll('#role-grid .role-option');
  roleOptions.forEach(function(btn) {
    btn.addEventListener('click', function() {
      roleOptions.forEach(function(item) { item.classList.remove('active'); });
      btn.classList.add('active');
      var mfaPanel = document.getElementById('mfa-panel');
      if (btn.dataset.role === 'system' && mfaPanel) {
        mfaPanel.classList.add('open');
        var mfaInput = document.querySelector('#mfa-code input');
  ...
  form.addEventListener("submit", interceptSubmit(async () => {
    ...
    // 获取选中的角色
    var activeRole = document.querySelector('#role-grid .role-option.active');
    var role = activeRole ? activeRole.dataset.role : 'operator';

    const restore = setLoading(button, "校验中...");
    try {
      await auth.loginAdmin({ username, password, role: role });
...
function updateSidebarBadges(metrics) {
  var disputeBadge = document.querySelector("[data-nav-badge=\"disputes\"]");
  if (disputeBadge) {
    var count = Number(metrics.disputeCount ?? 0);
    if (count > 0) {
      disputeBadge.textContent = String(count);
      disputeBadge.style.display = "";
    } else {
...
async function loadAdminAuditLogs(state, adminSession) {
  ...
  if (table) {
    table.innerHTML = `<tr><td colspan="7">${adminPanelLoadingHtml("正在加载审计日志。")}</td></tr>`;
  }
  try {
...
function renderAdminAuditLogs(payload, state, adminSession) {
  ...
  if (logs.length === 0) {
    table.innerHTML = `<tr><td colspan="7">暂无符合条件的审计日志。</td></tr>`;
  } else {
...
function adminAuditLogRowHtml(log) {
  ...
  return `
    ...
    <tr class="au-detail-row" id="audit-detail-${escapeHtml(log.auditId)}"><td colspan="7"><div class="au-detail-content"><div class="diff-block">${escapeHtml(detail)}</div></div></td></tr>
  `;
}
...
async function loadAdminAiLogs(state, adminSession) {
  ...
  if (table) {
    table.innerHTML = `<tr><td colspan="7">${adminPanelLoadingHtml("正在加载 AI 调用日志。")}</td></tr>`;
  }
  try {
...
function renderAdminAiLogs(payload, state, adminSession) {
  ...
  table.innerHTML = logs.length === 0
    ? `<tr><td colspan="7">暂无符合条件的 AI 调用日志。</td></tr>`
...
function adminAiLogRowHtml(item) {
  ...
  return `
    ...
    <tr class="al-detail-row" id="ai-log-detail-${escapeHtml(item.callId)}"><td colspan="7"><div class="al-detail-content">
...
function renderAdminUsers(payload, state, adminSession) {
  ...
  if (users.length === 0) {
    ...
    tbody.querySelectorAll("[data-admin-view]").forEach((button) => {
      button.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopImmediatePropagation();
        var userId = button.dataset.adminView;
        if (userId) {
          window.open("/users/" + encodeURIComponent(userId), "_blank");
        }
      }, true);
    });
  }
...
function applyAdminTransactionsControls(state) {
  ...
  setSelectValueSilent("typeFilter", state.type);
  setSelectValueSilent("statusFilter", state.status);
  setSelectValueSilent("riskFilter", state.risk);
}
...
function renderAdminTransactions(payload, state, adminSession) {
  ...
  if (transactions.length === 0) {
    tbody.innerHTML = `<tr><td colspan="7"><div class="tx-empty"><p>没有符合条件的流水。</p></div></td></tr>`;
...
function setSelectValueSilent(id, value) {
  const element = document.getElementById(id);
  if (element) {
    element.value = value ?? "";
  }
...
async function hydrateAdminStatsRoute(session) {
  ...
  applyAdminIdentity(adminSession.user);
  installAdminStatsControls(adminSession);
  try {
    const payload = await api.admin.stats(adminSession.token);
    window.__adminStats = payload;
...
function installAdminStatsControls(adminSession) {
  if (document.body.dataset.adminStatsBound === "true") {
    return;
  }
  document.body.dataset.adminStatsBound = "true";

  var timeButtons = document.querySelectorAll(".time-range button");
    ...
    button.addEventListener("click", function(event) {
      ...
      if (text === "导出") {
        if (!window.__adminStats) {
          showGlobalMessage("暂无统计数据可导出。", "error");
          return;
        }
        exportAdminStatsCsv();
        showGlobalMessage("统计数据已导出为 CSV 文件。", "success");
        return;
      }
      
      if (text === "刷新") {
        timeButtons.forEach(function(btn) { btn.disabled = true; });
        button.textContent = "刷新中...";
        window.__adminStats = null;
        api.admin.stats(adminSession.token).then(function(payload) {
          window.__adminStats = payload;
          renderAdminStats(payload);
          timeButtons.forEach(function(btn) { btn.disabled = false; });
      ...
      timeButtons.forEach(function(btn) { btn.classList.remove("active"); });
...
function exportAdminStatsCsv() {
  ...
  var rows = [["指标", "数值"]];
  var kpis = stats.kpis || {};
  rows.push(["用户数", kpis.userCount || 0]);
  rows.push(["流通时间币", kpis.circulatingCoins || 0]);
  rows.push(["完成订单数", kpis.completedOrderCount || 0]);
  rows.push(["纠纷率", (kpis.disputeRate || 0) + "%"]);
  rows.push(["平均评分", kpis.averageCredit || 0]);
  rows.push([""]);
  rows.push(["月份", "新增用户", "累计用户"]);
  (stats.userGrowth || []).forEach(function(item) {
    rows.push([item.month, item.newUsers, item.totalUsers]);
  });
  rows.push([""]);
  rows.push(["月份", "订单数"]);
  (stats.orderTrend || []).forEach(function(item) {
    rows.push([item.month, item.orders]);
  });
  downloadCsv("admin-stats-" + Date.now() + ".csv", rows);
}
...
function adminDisputeEvidencePanel(title, user, claim, evidence) {
  return `
    <div class="evidence-panel">
      ...
      <div class="ep-claim">${escapeHtml(claim || "未提供详细说明")}</div>
      <div class="ep-files">
        ${(evidence.length ? evidence : [{ content: "暂无附件", attachments: [] }]).map((item) => {
          ...
          return attachments.map((attachment) => {
            ...
            const url = attachment.url || (fid ? "/api/files/" + encodeURIComponent(fid) : "");
            if (url) {
              return '<a href="' + url + '" target="_blank" class="ep-file-img"><img src="' + url + '" style="max-width:100%;max-height:300px;border-radius:8px;display:block;" alt=""></a>';
            }
...
function installPublicProfileActions(payload, userSession) {
  ...
  document.querySelectorAll(`a[href="credit.html"], a[href="/credit"]`).forEach((link) => {
    var existing = new URLSearchParams(link.href.includes("?") ? link.href.split("?")[1] : "");
    existing.set("userId", encodeURIComponent(targetUserId));
    link.setAttribute("href", `/credit?${existing.toString()}`);
  });

...
const ADMIN_ROLES = [
  { role: "operator", label: "运营", icon: "⊞" },
  { role: "arbiter", label: "仲裁", icon: "⚖" },
  { role: "system", label: "系统", icon: "⚙" }
];
...
export function LoginPage({ admin = false }: { api: ApiClient; admin?: boolean }) {
  ...
  const [selectedRole, setSelectedRole] = React.useState("operator");
  ...
  return (
    <main className="auth-page">
      <form className="auth-card" onSubmit={async (event) => {
        ...
        try {
          if (admin) {
            await auth.loginAdmin({ username: form.get("username"), password: form.get("password"), role: selectedRole });
        ...
        {admin ? (
          <Field label="登录角色">
            <div className="admin-role-grid">
              {ADMIN_ROLES.map(({ role, label, icon }) => (
                <button
                  type="button"
                  key={role}
                  className={`admin-role-option${selectedRole === role ? " active" : ""}`}
                  onClick={() => setSelectedRole(role)}
                >
                  <span className="admin-role-icon">{icon}</span>

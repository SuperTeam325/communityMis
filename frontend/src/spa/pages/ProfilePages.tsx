...
export function ProfilePage({ api }: { api: ApiClient }) {
  ...
  const handleAvatarUpload = async (formData: FormData) => {
    ...
    try {
      ...
      if (!fileId) {
        setUploadError("文件上传成功但未返回文件 ID，请重试。");
        return;
      }
      const avatarResult = await api.users.avatar(fileId);
      const updatedUser = (avatarResult as Record<string, unknown>)?.user as Record<string, unknown> | null;
      if (updatedUser && state.data) {
        state.setData({ ...state.data, user: updatedUser });
      }
      await auth.refresh();
    } catch (error) {
      setUploadError(error instanceof Error ? error.message : "头像更新失败，请重试。");
    } finally {
      setUploading(false);
    }
  };
    ...
    <>
      ...
      <StateView loading={state.loading} error={state.error} empty={!user}>
        <section className="panel profile-panel">
          ...
          <FileUpload purpose="avatar" businessType="user" visibility="public" onUploaded={handleAvatarUpload} />
          {uploading && <small>正在上传并更新头像...</small>}
